...
def load_image_records(sqlite_path, minimum_tag_count):
    ...
    cursor.execute(
        "SELECT md5, file_ext, tag_string FROM posts WHERE (file_ext = 'png' OR file_ext = 'jpg' OR file_ext = 'jpeg' OR file_ext = 'webp' OR file_ext = 'gif' OR file_ext = 'bmp') AND (tag_count_general >= ?) ORDER BY id",
        (minimum_tag_count,),
    )
